...
//
// 实现细节上不依赖具体提示文案，只钉业务码与「错误信息被原样透传」，
// 避免测试绑死在会漂移的字符串上。
//
// 补充背景：曾把 40 处 c.ShouldBindJSON + response.BadRequest 收敛成 shared.BindJSON，
// 但那会把各接口的业务语义文案（「邮箱格式错误!」「标题和内容不能为空」等）压成一句
// 通用的「请求参数错误」，而前端会直接展示这个文案 → 已回退，保留各接口自己的文案。
// 所以这里的断言口径是「不钉文案」，而不是「文案已统一」。
